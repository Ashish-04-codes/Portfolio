            ...
            <nav className="border-t border-ink py-3 hidden md:flex justify-center gap-8 lg:gap-12 font-bold font-headline uppercase tracking-widest text-sm transition-colors duration-300 no-print">
                {NAV_LINKS.map((link) => (
                    <button
                        ...
                        className={`hover:underline decoration-2 underline-offset-4 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink rounded px-1 ${currentPage === link.id ? 'underline' : ''
                            }`}
                ...
                <nav className="md:hidden border-t border-ink bg-newsprint py-4 px-6 pl-16 flex flex-col gap-4 font-bold font-headline uppercase tracking-widest text-lg no-print shadow-lg">
                    {NAV_LINKS.map((link) => (
                        <button
                            ...
                            className={`text-left hover:text-ink-light transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink rounded px-1 ${currentPage === link.id ? 'underline decoration-2 underline-offset-4' : ''
                                }`}
                            aria-current={currentPage === link.id ? 'page' : undefined}
                        >
                            {link.label}
