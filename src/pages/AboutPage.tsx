...
const AboutPage: React.FC = () => {
    ...
    useEffect(() => {
        const handleStorageChange = () => {
            Promise.all([
                ...
                logger.debug('[AboutPage] Soft refresh', { profile: profileData ? { name: profileData.name, title: profileData.title, location: profileData.location } : null });
    ...
    const loadData = async () => {
        ...
        logger.debug('[AboutPage] Loaded profile', { profile: profileData ? { name: profileData.name, title: profileData.title, location: profileData.location } : null });
        setProfile(profileData);
        ...
        <article className="bg-surface p-4 md:p-8 shadow-sm border border-ink/10 transition-colors">
            ...
            <header className="mb-8 border-b-2 border-ink pb-6 text-center">
                <div className="flex items-center justify-center gap-4 mb-2">
                    ...
                    <span className="font-mono text-xs font-bold uppercase tracking-widest bg-ink text-newsprint px-2 py-0.5">Profile</span>
                    <span className="h-px w-12 bg-ink"></span>
                </div>
                <h2 className="text-5xl md:text-7xl lg:text-8xl font-black font-headline uppercase leading-[0.9] tracking-tighter mb-4">
                    The Developer<br />
                    <span className="italic font-headline font-light text-4xl md:text-6xl lg:text-7xl normal-case tracking-normal block mt-2">Behind The Code</span>
                </h2>
                ...
                <div className="md:col-span-5 space-y-8">
                    {profile.sections && profile.sections.length > 0 ? (
                        ...
                        <div className="space-y-2">
                            <h3 className="font-mono text-sm font-bold uppercase tracking-widest border-b border-ink inline-block mb-1">About Me</h3>
                            <p className="font-body text-lg leading-relaxed text-ink/90 text-justify">
                    ...
                    {skills.length > 0 && (
                        <div className="text-center space-y-2 opacity-80 mt-4">
                            ...
                            <div className="flex flex-wrap justify-center gap-2 text-xs font-mono uppercase">
                                {skills.map(skill => (
                                    <span key={skill.id} className="border border-ink px-2 py-1 hover:bg-ink hover:text-newsprint transition-colors cursor-default">
                                        {skill.name}
                                    </span>
                                ))}
                            </div>
