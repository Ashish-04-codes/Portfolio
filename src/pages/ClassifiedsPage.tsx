...
import { ArrowRight, ArrowLeft, ExternalLink, Code, FolderOpen, X, Globe, Github, Layers, Cpu } from 'lucide-react';
import { projectService } from '../services';
import { Project } from '../models';
import { useAppNavigate } from '../hooks/useAppNavigate';

// Data Models
type ProjectLayout = 'featured' | 'standard' | 'inverted' | 'text-only' | 'sidebar-image' | 'placeholder';
...
const ProjectModal: React.FC<ProjectModalProps> = ({ project, onClose }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className={`relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-newsprint border-4 border-double border-ink shadow-[10px_10px_0px_0px_rgba(0,0,0,0.5)] flex flex-col duration-300 ${isClosing
                ...
                <div className="sticky top-0 z-10 bg-newsprint border-b border-ink p-4 flex justify-between items-center select-none">
                    <div className="flex gap-4 items-center">
                        ...
                        <span className="font-mono text-xs font-bold hidden sm:inline-block">Filed: {project.year}</span>
                    </div>
                    <button
                        onClick={handleClose}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                        ...
                        <div className="md:col-span-4 space-y-8">
                            ...
                            <div className="bg-surface border border-ink p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {project.techStack?.map((tech) => (
                                        <span key={tech} className="bg-ink text-newsprint px-2 py-1 font-mono text-xs font-bold border border-ink hover:bg-transparent hover:text-ink transition-colors cursor-default">
                                            {tech}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div className="border-2 border-dashed border-ink p-4 text-center opacity-50 rotate-3 select-none">
                                ...
                                <div className="font-mono text-[10px] uppercase mt-1">Daily Developer Eng. Dept.</div>
                            </div>
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, onSelect }) => {
    switch (project.layout) {
        case 'featured':
            return (
                <div className="col-span-1 md:col-span-2 row-span-2 border-r border-b border-ink p-0 flex flex-col group cursor-pointer" onClick={() => onSelect(project)}>
                    ...
                    <div className="p-6 flex flex-col h-full justify-between bg-newsprint group-hover:bg-surface transition-colors">
                        <div>
                            <div className="flex justify-between items-start mb-2">
                                <span className="bg-ink text-newsprint text-[10px] font-mono px-1 py-0.5 uppercase tracking-wider">Showcase</span>
                                <span className="font-mono text-xs font-bold">{project.year}</span>
                            </div>
                            <h3 className="text-3xl font-headline font-bold mb-2 leading-tight group-hover:underline decoration-2 underline-offset-4">{project.title}</h3>
                        ...
                        <div className="mt-4 pt-4 border-t border-dotted border-ink flex justify-between items-center">
                            <span className="font-mono text-xs uppercase font-bold text-ink-light">Category: {project.category}</span>
        ...
        case 'inverted':
            return (
                <div className="border-r border-b border-ink p-6 bg-ink text-newsprint cursor-pointer group" onClick={() => onSelect(project)}>
                    <div className="border-2 border-current p-2 h-full flex flex-col justify-center text-center group-hover:bg-newsprint group-hover:text-ink transition-colors">
                        <h4 className="font-mono font-bold text-sm uppercase tracking-widest mb-2 border-b border-current pb-2">{project.category}</h4>
                        <p className="font-headline text-base italic leading-relaxed mb-4">
                            "{project.title}" <br />
                            {project.shortDesc}
                        </p>
                        <button className="text-xs font-mono uppercase underline decoration-1 underline-offset-2 hover:no-underline flex items-center justify-center gap-2"><Code size={12} /> View Details</button>
                    </div>
                </div>
            );
        ...
        case 'text-only':
            return (
                <div className="border-r border-b border-ink p-4 cursor-pointer hover:bg-surface transition-colors group" onClick={() => onSelect(project)}>
                    <div className="flex justify-between items-baseline border-b border-ink pb-1 mb-2">
                        <h4 className="font-bold text-lg font-headline group-hover:underline">{project.title}</h4>
                        <span className="text-[10px] font-mono">{project.year}</span>
                    </div>
                    <p className="text-xs font-body leading-snug mb-2">
                        <span className="float-left text-3xl font-black mr-1 leading-[0.8] font-headline">{project.shortDesc.charAt(0)}</span>
        ...
        case 'sidebar-image':
            return (
                <div className="border-r border-b border-ink p-4 flex flex-row gap-3 cursor-pointer hover:bg-surface transition-colors group" onClick={() => onSelect(project)}>
                    <div className="w-16 h-full bg-newsprint-dark halftone border-r border-ink shrink-0">
                        <img src={project.image} className="w-full h-full object-cover" alt={project.title} loading="lazy" />
                    </div>
                    <div className="flex flex-col justify-between">
                        <div>
                            <h4 className="font-bold text-sm uppercase tracking-wide group-hover:underline">{project.title}</h4>
        ...
        default:
            return (
                <div className="border-r border-b border-ink p-4 flex flex-col cursor-pointer hover:bg-surface transition-colors group" onClick={() => onSelect(project)}>
                    {project.image && (
                        <div className="mb-3 border border-ink p-1 bg-surface halftone h-32 overflow-hidden">
                            <img src={project.image} className="w-full h-full object-cover grayscale group-hover:scale-110 transition-transform duration-700" alt={project.title} loading="lazy" />
                        </div>
                    )}
                    <h4 className="font-bold text-xl uppercase border-b border-ink pb-1 mb-2 font-headline group-hover:underline">{project.title}</h4>
                    <p className="text-sm font-body leading-tight mb-3 text-justify">
                        {project.shortDesc}
                    </p>
        ...
        const handleStorageChange = () => {
            projectService.getAll().then(projectsData => {
                const mappedProjects: ProjectItem[] = projectsData.map((project, index) => ({
                    ...project,
                    layout: (index === 0 ? 'featured' : index % 4 === 0 ? 'inverted' : 'standard') as ProjectLayout
                }));
    ...
    if (projects.length === 0) {
        return (
            <div className="text-center py-12 border border-ink p-8">
                ...
                <p className="text-xs text-ink/60">Add projects in the admin panel to showcase your work.</p>
            </div>
        );
    }
    ...
    return (
        <div>
            ...
            <div className="border-b-4 border-ink mb-2 pb-2 flex flex-col md:flex-row justify-between items-end gap-4">
                <div>
                    <h2 className="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-[0.8] font-headline">Project Catalog</h2>
                    <p className="text-sm md:text-base font-mono uppercase tracking-widest mt-2">A Collection of Digital Works & Case Studies</p>
                </div>
                <div className="text-right hidden md:block">
                    <p className="text-xs font-headline italic max-w-xs">
                        "Showcasing the finest digital craftsmanship, tailored code, and bespoke interfaces. Available for detailed review."
                    </p>
                </div>
            </div>
            ...
            <div className="flex justify-between items-center border-t border-b border-ink py-2 mt-4 font-mono text-xs uppercase">
                <button className="flex items-center hover:underline" onClick={() => navigate('front')}><ArrowLeft size={14} className="mr-1" /> Home</button>
                <span>Catalog Page 1 of 1</span>
                <button className="flex items-center hover:underline opacity-50 cursor-not-allowed">Next <ArrowRight size={14} className="ml-1" /></button>
            </div>
        </div>
    );
};
